(ns borderops.operation
  "OperationActor -- one cross-border request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (CrossBorderAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the CrossBorderGovernor
  ...
  One graph run = one cross-border request (intake -> advise -> govern ->
  ...
  human customs/trade compliance reviewer. The approver resumes with
  `{:approval {:status :approved :by \"..\"}}` (or :rejected).

  Because `:propose-hs-classification` always escalates (governor AND
  phase, independently), the ONLY path by which an HS heading enters the
  classification directory runs through this interrupt -- customs
  classification is a legal act, so a named human accepting it is
  structurally unavoidable rather than merely conventional."
...
(defn build
  ...
  `borderops.store/Store`).
  opts:
    :advisor      -- a `borderops.advisor/Advisor` (default: mock-advisor)
